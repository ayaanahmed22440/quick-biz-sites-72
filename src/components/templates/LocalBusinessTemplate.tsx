...
/**
 * The single rendering engine behind every WebWarheads design.
 * The preset chosen in `content.templateId` decides imagery, accent and the
 * hero arrangement; every section below is shared.
 */
export function LocalBusinessTemplate({
  ...
  const quoteForm = (
    <div id="quote" className="rounded-xl border border-black/10 bg-white p-5 shadow-sm sm:p-6">
      ...
      {sent ? (
        <p className="mt-6 rounded-md bg-black/5 p-4 text-sm">
  ...
  return (
    <div style={{ ...body, color: "#16181d", background: "#ffffff" }} className="min-h-full">
        ...
        <section className="relative">
          ...
          <div className="absolute inset-0 flex items-center">
            <div className="mx-auto w-full max-w-6xl px-4 text-white sm:px-6">
              <h1 style={heading} className="max-w-2xl text-3xl leading-tight sm:text-4xl lg:text-5xl">
      ...
      {layout === "banner" ? (
        <section style={{ background: brand }} className="text-white">
          <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:items-center lg:py-14">
            <div>
              ...
              <p className="mt-4 text-base text-white/85 sm:text-lg">{content.hero.subheadline}</p>
              <ul className="mt-6 space-y-2">
                {content.hero.trustPoints.filter(Boolean).map((point) => (
                  <li key={point} className="flex items-start gap-2 text-sm sm:text-base">
                    <span aria-hidden className="mt-1.5 inline-block h-2 w-2 shrink-0 rounded-full bg-white" />
                    <span>{point}</span>
                  </li>
                ))}
              </ul>
              <a
                href="#quote"
                className="mt-6 inline-block rounded-md bg-white px-5 py-3 text-sm font-semibold"
                style={{ color: brand }}
              >
                {content.hero.primaryCta}
              </a>
            ...
            <img
              src={heroImage}
              alt=""
              className="h-64 w-full rounded-xl object-cover shadow-lg sm:h-80 lg:h-96"
            />
          </div>
        </section>
      ) : null}
      ...
      {gallery.length > 0 ? (
        <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:py-16">
          ...
          <p className="mt-2 max-w-2xl text-black/70">{content.gallery.intro}</p>
      ...
      <section className="border-t border-black/10">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-2 lg:items-center lg:py-16">
          ...
          <div>
            ...
            <p className="mt-4 whitespace-pre-line leading-relaxed text-black/75">
              {content.about.body}
            </p>
            <h3 style={heading} className="mt-8 text-lg">
      ...
      {/* Quote form for non-split layouts */}
      {layout !== "split" ? (
        <section className="border-t border-black/10 bg-[#f7f8fa]">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:py-16">{quoteForm}</div>
        </section>
      ) : null}
            ...
            <div>
              ...
              <ul className="mt-3 space-y-1 text-sm">
                {hours.map((h) => (
                  <li
                    key={h.day_of_week}
                    className="flex justify-between gap-4 border-b border-white/20 py-1"
                  >
